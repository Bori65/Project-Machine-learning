...
df = df.drop(['rerun_ID','obj_ID','run_ID','field_ID','spec_obj_ID','fiber_ID'],axis=1)
results = {}
for attribute in df.keys():
    target = 'class'
    x = df.drop([attribute, target],axis=1).values[:10000]
    ...
    plt.title(title)
    plt.legend()
    plt.savefig('Stellar_loss_excluded_attributes\stellar_'+ attribute +'.png')
    plt.show()
    results[attribute] = mse

with open('stellar2_results.txt', 'w') as file:
    # Write header
